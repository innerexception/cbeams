...
type FeatureKind = 'hill' | 'valley' | 'spur'
const FEATURE_KINDS:Array<FeatureKind> = ['hill', 'valley', 'spur']
...
const makeFeature = (kind:FeatureKind, size:number, center:number):TerrainFeatureInstance => {
    ...
    return makeSpurFeature(size, center)
}

// At least 2 distinct kinds every match (the terrain feature mix itself varies match to match, not
// just each feature's own placement) — up to all 3, shuffled so there's no fixed ordering bias.
...
const buildTerrain = (size:number):TerrainData => {
    ...
    // guessed size — a Valley/Spur's line segment can point in any random direction from its anchor
